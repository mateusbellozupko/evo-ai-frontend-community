...
export interface WahaAuthorizationResponse {
  session_name: string;
  // WAHA session status as reported by the WAHA server at session creation
  // (e.g. 'STARTING', 'SCAN_QR_CODE', 'WORKING'). Absent when the WAHA
  // response body carries no status.
  status?: string;
...
export interface WahaQrCodeResponse {
  // Data URL of the pairing QR code, as returned by /waha/qrcodes/:inbox_id.
  qr_data_url?: string;
  error?: string;
}

// The logout endpoint answers with an empty body on success; only the error
// envelope carries a payload.
export interface WahaLogoutResponse {
  error?: string;
}
